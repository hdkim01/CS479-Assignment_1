...
class PointNetFeat(nn.Module):
    ...
    def forward(self, pointcloud):
        ...
        pointcloud = pointcloud.reshape(B, -1, 64).transpose(2, 1)
        transform64_64 = self.stn64(pointcloud)
        pointcloud = pointcloud.transpose(2, 1)
        pointcloud = torch.bmm(pointcloud, transform64_64)

        pointcloud = pointcloud.reshape(-1, 64)
        pointcloud = self.mlp2(pointcloud)

        pointcloud = pointcloud.reshape(B, -1, 1024)
        pointcloud = torch.max(pointcloud, 1)[0]
...
class PointNetCls(nn.Module):
    def __init__(self, num_classes, input_transform, feature_transform):
        ...
        self.mlp = nn.Sequential(nn.Linear(1024, 512), nn.BatchNorm1d(512), nn.ReLU(),
    ...
    def forward(self, pointcloud):
        ...
        pointcloud = self.mlp(pointcloud)
