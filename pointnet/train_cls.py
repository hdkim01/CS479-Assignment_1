...
def step(points, labels, model):
    ...
    outputs, transform64_64 = model(points)
    criterion = nn.CrossEntropyLoss()
    point_loss = criterion(outputs, labels)
    regularization_loss = get_orthogonal_loss(transform64_64)
...
def main(args):
    global device
    device = "cpu" if args.gpu == -1 else f"cuda"
    print(device)
